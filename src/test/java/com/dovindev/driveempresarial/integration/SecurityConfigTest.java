...
/**
 * Verifies SecurityConfig permits access to /api/drive/** endpoints and that CORS headers are
 * properly configured.
 */
@WebMvcTest
...
class SecurityConfigTest {
  ...
  void shouldPermitAccessToDriveProcessesEndpoint() throws Exception {
    ...
    mockMvc.perform(get("/api/drive/processes")).andExpect(status().isNotFound());
  }

  @Test
  void shouldPermitAccessToDriveDocumentsEndpoint() throws Exception {
    mockMvc.perform(get("/api/drive/documents")).andExpect(status().isNotFound());
  }
